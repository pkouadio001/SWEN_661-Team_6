...
describe("registerIpcHandlers", () => {
  ...
  test("registers GET_VERSION handler", async () => {
    ...
    const handler = mockHandle.mock.calls.find(
      ([channel]) => channel === IPC.GET_VERSION
    )?.[1];
  ...
  test("registers QUIT_APP listener and quits app when invoked", () => {
    ...
    const quitListener = mockOn.mock.calls.find(
      ([channel]) => channel === IPC.QUIT_APP
    )?.[1];
  ...
  test("registers NAVIGATE listener and forwards payload to renderer", () => {
    ...
    const getWindow = jest.fn(() => mockWindow as any);
    ...
    const navigateListener = mockOn.mock.calls.find(
      ([channel]) => channel === IPC.NAVIGATE
    )?.[1];
    ...
    expect(sendMock).toHaveBeenCalledWith(IPC.NAVIGATE, payload);
  });
