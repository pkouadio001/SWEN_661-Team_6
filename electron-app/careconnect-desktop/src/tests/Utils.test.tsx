...
const isSixDigits = (pin: unknown) => {
  ...
  return /^[0-9]{6}$/.test(pin);
    ...
    describe('Edge Cases', () => {
      test('rejects null', () => {
        expect(isSixDigits(null as any)).toBe(false);
      });

      test('rejects undefined', () => {
        expect(isSixDigits(undefined as any)).toBe(false);
      });

      test('rejects number type', () => {
        expect(isSixDigits(123456 as any)).toBe(false);
      });

      test('rejects object', () => {
        expect(isSixDigits({} as any)).toBe(false);
      });
