...
describe("main process lifecycle", () => {
    ...
    test("registers activate handler", async () => {
        await importMainFresh();
        const activateCall = appOnMock.mock.calls.find(([event]) => event === "activate");
    ...
    test("recreates window on activate when no windows exist", async () => {
        getAllWindowsMock.mockReturnValue([]);
        await importMainFresh();
        const activateHandler = appOnMock.mock.calls.find(([event]) => event === "activate")?.[1];
    ...
    test("quits app on window-all-closed when not on darwin", async () => {
        ...
        await importMainFresh();
        const windowClosedHandler = appOnMock.mock.calls.find(([event]) => event === "window-all-closed")?.[1];
