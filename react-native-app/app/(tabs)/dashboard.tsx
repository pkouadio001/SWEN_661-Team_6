...
export default function DashboardScreen() {
 ...
 const NavItem = ({ label, icon, to, active, badge, onPress, color }: NavItemProps) => {
  ...
  const accessibilityLabel =
    badge && badge > 0 ? `${label}. ${badge} ${badge > 1 ? "items" : "item"}` : label;

  const isAction = !!onPress; // Logoff etc.
  const role: "tab" | "button" = isAction ? "button" : "tab";
  ...
  return (
    <Pressable
      ...
      accessibilityHint={isAction ? `Perform ${label} action` : `Navigate to ${label} tab`}
      accessibilityState={role === "tab" ? { selected: !!active } : undefined}
      style={styles.navItem}
    >
      <View style={styles.navIconWrap} accessible={false}>
        <Text style={[styles.navIcon, { color: iconColor }]} accessible={false}>
          {icon}
        </Text>
      ...
      <Text style={[styles.navLabel, { color: iconColor }]} accessible={false}>
        {label}
      </Text>
    </Pressable>
  );
